...
function initialise(chessboard){
    ...
    let two_bish=2*Math.floor(Math.random()*4)+1;
        chessboard[one_bish][7] = new chess_piece(one_bish, 7, "l", "b", 0);
        chessboard[one_bish][7].draw();
        chessboard[one_bish][0] = new chess_piece(one_bish, 0, "d", "b", 0);
        chessboard[one_bish][0].draw();
        chessboard[two_bish][7] = new chess_piece(two_bish, 7, "l", "b", 0);
        chessboard[two_bish][7].draw();
        chessboard[two_bish][0] = new chess_piece(two_bish, 0, "d", "b", 0);
    ...
    for(let i = 0; i<3; i++){
        ...
        chessboard[rand_row][7]= new chess_piece(rand_row,7,"l",nnq[i], 0);
        chessboard[rand_row][7].draw();
        chessboard[rand_row][0]= new chess_piece(rand_row,0,"d",nnq[i], 0);
        chessboard[rand_row][0].draw();
        remove_from(free_squares,rand_row);
    }
    let rkr=["r","k","r"];
    for(let i = 0; i<3; i++){     
        chessboard[free_squares[i]][7]= new chess_piece(free_squares[i],7,"l",rkr[i], 0);
        chessboard[free_squares[i]][7].draw();
        chessboard[free_squares[i]][0]= new chess_piece(free_squares[i],0,"d",rkr[i], 0);
